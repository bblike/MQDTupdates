...
C
 ...
 2104   FORMAT(1X,'*** Error in SBOUND: User radial grid with',
     1    ' more than ',I5,' data points.')
        STOP
      ENDIF
      T=MAX(0.5D0*EPS,1.0D-10) ! changed!!!
      DO I=1,NVT
        R(I)=MAX(RG(I), 1.0D-10)
        IND(I)=I
      ENDDO
      NRT=NVT
C
...
C  ************  New shot.
C
      write(6,*) 'repeat calculation begin'
 2    CONTINUE
      IF(E.GT.-1.0D-20) THEN
        IER=2
        WRITE(6,1002)
...
C
 ...
 1005   FORMAT(1X,'*** Error 5 in SBOUND: E out of range.'/5X,
     1    '(Accumulated round-off errors?).')
      ...
      EO=E
      E=EP
      ! write(6,*) 'flag3: before convergence test'
      IF(MIN(ABS(DE),ABS(E-EO)).GT.ABS(E*DELL)) THEN
        ! write(6,*) 'flag4: convergence test'
        GO TO 2
      ENDIF
C  ****  Normalization
      FACT=1.0D0/SQRT(SUM)
